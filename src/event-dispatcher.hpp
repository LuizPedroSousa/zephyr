...
public:
  ...
  void subscribe(Args &&...args) {
    ...
    ZEPH_ENSURE(!emplaced_listener.second, "Error creating new Listener!");
    ...
    ZEPH_ENSURE(!emplaced_event.second, "Error creating new Event Listener!");
  }
  ...
  template <typename L, typename E, typename F> void attach(F callback) {
    auto typed_callback = [callback](Event *event) {
      ...
      if (typed_event) {
        ...
        ZEPH_EXCEPTION("Event type mismatch! Expected: " +
    ...
    auto emplaced_listener =
        m_listeners.emplace(listener_id, std::move(listener));
    ZEPH_ENSURE(!emplaced_listener.second,
